...
##########################################################################################################################
        ...
        self.lora_Cu[adapter_name] = nn.Conv2d(24, r, kernel_size=3, stride=2, padding=1, bias=False)
        self.conv1 = self.conv1.cuda()
        self.lora_Cu = self.lora_Cu.cuda()
...
######################################################### EOF CONV #########################################################
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            #print("lora_A_keys----", lora_A_keys)
            #print("active_adapters----", self.active_adapters)
            #print(f"lora variant is: {self.lora_variant}")
            ...
            for active_adapter in self.active_adapters:
                ...
                lora_Cu = self.lora_Cu[active_adapter]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                ...
                # TODO: check what happens when merging c-dora & c-lora adapaters
                #print(f"active_adapter: {active_adapter}")
                #print(f"self.lora_variant: {self.lora_variant}")
                #print(f" length of self.lora_variant: {len(self.lora_variant)}")
                # print("conv1 shape after merging: ", conv1.weight.shape)
                #import time; time.sleep(10000)
                # Goes through this if condition when merging multiple adapters??
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    ...
                    #print("c1_op shape----", c1_op.shape)
                    #print("lora_Cu shape----", lora_Cu.weight.shape)
                    #import time; time.sleep(10000)
                    c1_op = lora_Cu(c1_op)

                    # Reshape back
                    C_out = c1_op.shape[1]
                    conv_flat = c1_op.permute(0, 2, 3, 1).reshape(B_new, -1, C_new) # C_out or C_new
                    ...
                    # Get merged rank from lora_A
                    merged_rank = lora_A.weight.shape[0]  # lora_A output dimension = merged rank
                    C_out = conv_result.shape[2]  # Current channels from conv output
                    #print("merged_rank----", merged_rank)
                    #print("C_out----", C_out)
                    #import time; time.sleep(10)
                    
                    # Project conv_result from C_out to merged_rank if needed
                    if C_out != merged_rank and False:
                        # Reshape for projection: [L*B, C_out]
                        L_orig, B_orig, _ = conv_result.shape
                        conv_result_flat = conv_result.reshape(-1, C_out)  # [L*B, C_out]
                        
                        # Create projection: map from C_out (24) to merged_rank (32)
                        # Use a simple linear projection (this could be learnable)
                        # For now, use identity-like projection with padding/truncation
                        if merged_rank > C_out:
                            # Pad with zeros: [merged_rank, C_out]
                            proj_weight = torch.eye(C_out, device=conv_result.device, dtype=conv_result.dtype)
                            padding = torch.zeros(merged_rank - C_out, C_out, device=conv_result.device, dtype=conv_result.dtype)
                            proj_weight = torch.cat([proj_weight, padding], dim=0)  # [merged_rank, C_out]
                        else:
                            # Truncate: [merged_rank, C_out]
                            proj_weight = torch.eye(merged_rank, device=conv_result.device, dtype=conv_result.dtype)
                            if C_out > merged_rank:
                                # Take first merged_rank columns
                                proj_weight = torch.cat([proj_weight, torch.zeros(merged_rank, C_out - merged_rank, 
                                                                 device=conv_result.device, dtype=conv_result.dtype)], dim=1)
                        
                        # Project: [L*B, C_out] @ [C_out, merged_rank] = [L*B, merged_rank]
                        conv_result_proj = F.linear(conv_result_flat, proj_weight)
                        conv_result = conv_result_proj.reshape(L_orig, B_orig, merged_rank)  # [L, B, merged_rank]
