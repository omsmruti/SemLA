    ...
    **Attributes**:
    ...
    def add_weighted_adapter(
        ...
        Args:
            ...
            merge_conv_lora (`bool`, *optional*):
                Whether to merge Conv-LoRA MoE components (gating and experts) when using `cat` combination.
                If False, Conv-LoRA is disabled for the merged adapter and only lora_A/lora_B are used.
                Defaults to True.
        """
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                ...
                if combination_type == "cat":
                    ...
                    # Handle Conv-LoRA MoE components for "cat" combination.
                    if hasattr(target, 'lora_moe_gating') and adapter_name in target.lora_moe_gating:
                        if merge_conv_lora:
                            print("merging conv lora components")
                            # Merge: Concatenate gating weights and create block-diagonal conv experts.
                            target_gating = target.lora_moe_gating[adapter_name]
                            target_experts = target.lora_moe_experts[adapter_name]
                            num_experts = len(target_experts)
                            
                            # Collect gating weights from source adapters
                            w_gates_list, w_noises_list = [], []
                            # Collect conv weights for block-diagonal construction
                            all_expert_conv_weights = [[] for _ in range(num_experts)]
                            all_expert_conv_biases = [[] for _ in range(num_experts)]
                            
                            for adapter, weight in zip(adapters, weights):
                                if adapter in target.lora_moe_gating:
                                    src_gating = target.lora_moe_gating[adapter]
                                    src_experts = target.lora_moe_experts[adapter]
                                    scaling = target.scaling.get(adapter, 1.0)
                                    
                                    # Scale and collect gating parameters for concatenation
                                    w_gates_list.append(src_gating.w_gate.data * weight * scaling)
                                    w_noises_list.append(src_gating.w_noise.data * weight * scaling)
                                    
                                    # Collect expert conv weights for block-diagonal construction
                                    for i, expert in enumerate(src_experts):
                                        if i < num_experts:
                                            conv_layer = expert[0]  # Conv2d
                                            all_expert_conv_weights[i].append(conv_layer.weight.data * weight * scaling)
                                            all_expert_conv_biases[i].append(conv_layer.bias.data * weight * scaling)
                            
                            if len(w_gates_list) > 0:
                                # Concatenate gating weights along dim=0 (rank dimension)
                                # Each adapter's w_gate is [r, M], concat gives [new_rank, M]
                                merged_w_gate = torch.cat(w_gates_list, dim=0)
                                merged_w_noise = torch.cat(w_noises_list, dim=0)
                                
                                # Resize target gating parameters
                                target_gating.w_gate = torch.nn.Parameter(merged_w_gate)
                                target_gating.w_noise = torch.nn.Parameter(merged_w_noise)
                                
                                # Build block-diagonal conv weights for each expert
                                # Original: [r, r, 3, 3], new: [new_rank, new_rank, 3, 3]
                                for i, expert in enumerate(target_experts):
                                    if len(all_expert_conv_weights[i]) > 0:
                                        conv_layer = expert[0]  # Conv2d
                                        conv_weights = all_expert_conv_weights[i]
                                        conv_biases = all_expert_conv_biases[i]
                                        
                                        # Create block-diagonal weight matrix
                                        # Each block is [r, r, 3, 3], result is [new_rank, new_rank, 3, 3]
                                        num_blocks = len(conv_weights)
                                        block_size = conv_weights[0].shape[0]  # r
                                        new_size = num_blocks * block_size  # new_rank
                                        kH, kW = conv_weights[0].shape[2], conv_weights[0].shape[3]
                                        
                                        # Initialize with zeros
                                        block_diag_weight = torch.zeros(
                                            new_size, new_size, kH, kW,
                                            device=conv_weights[0].device,
                                            dtype=conv_weights[0].dtype
                                        )
                                        
                                        # Fill in block-diagonal
                                        for j, w in enumerate(conv_weights):
                                            start = j * block_size
                                            end = start + block_size
                                            block_diag_weight[start:end, start:end, :, :] = w
                                        
                                        # Concatenate biases
                                        merged_bias = torch.cat(conv_biases, dim=0)
                                        
                                        # Create new conv layer with correct dimensions
                                        ...
                                        new_conv.weight.data = block_diag_weight
                                        new_conv.bias.data = merged_bias
                                        
                                        # Replace the conv layer in the expert
                                        expert[0] = new_conv
                        else:
                            # Don't merge: Disable Conv-LoRA for the merged adapter, use only lora_A/lora_B
                            target.use_conv_lora[adapter_name] = False
                            if adapter_name in target.lora_moe_gating:
                                del target.lora_moe_gating[adapter_name]
                            if adapter_name in target.lora_moe_experts:
                                del target.lora_moe_experts[adapter_name]
