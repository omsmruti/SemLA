...
class LoraConfig(PeftConfig):
    ...
    use_conv_lora: bool = field(
        default=False,
        metadata={
            "help": (
                "Enable Conv-LoRA which adds convolutional MoE experts between LoRA A and B matrices. "
                "This captures local spatial patterns and is especially useful for vision tasks like "
                "semantic segmentation. Reference: https://arxiv.org/abs/2401.17868"
            )
        },
    )
    conv_lora_expert_num: int = field(
        default=8,
        metadata={
            "help": (
                "Number of MoE experts for Conv-LoRA. Each expert is a Conv2d layer that operates "
                "at different spatial scales via upsampling/downsampling."
            )
        },
    )
    conv_lora_topk: int = field(
        default=1,
        metadata={"help": "Number of experts to select per sample in Conv-LoRA gating (top-k selection)."},
    )
    conv_lora_noisy_gating: bool = field(
        default=True,
        metadata={"help": "Whether to use noisy gating during training for better load balancing across experts."},
    )
