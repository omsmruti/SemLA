        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                ...
                if combination_type == "cat":
                    loras_A, loras_B = [], []
                    #print('Inside cat combination type of add_weighted_adapter')
                    #print('adapters and weights are: ', adapters, weights)
                    ...
                    # TODO: merge the conv weights as well
                    if hasattr(target, 'conv1') and adapter_name in target.conv1:
                        # target.conv1 is moduledict list of conv layers
                        target_conv1 = target.conv1[adapter_name] # randomly initialized conv layer
                        #print(f'target_conv1 shape is: {target_conv1.weight.shape}')
                        conv1s = []
                        for adapter, weight in zip(adapters, weights):
                            if adapter in target.conv1:
                                conv1_weight = target.conv1[adapter].weight.data
                                if hasattr(target, 'scaling') and adapter in target.scaling:
                                    scaling = target.scaling[adapter]
                                else:
                                    scaling = 1.0
                                conv1s.append(conv1_weight * weight * scaling)
                        if len(conv1s) > 0:
                            # TODO: check whether to cat the convs or avg them up
                            merged_conv1 = torch.cat(conv1s, dim=1)
                            #merged_conv1 = sum(conv1s) / len(conv1s)
                            target_conv1.weight.data = merged_conv1
                        #print(f'target_conv1 shape after merging is: {target_conv1.weight.shape}')
                        #print('merged_conv1 is: ', merged_conv1.shape)
                        #import time; time.sleep(10)

                    if hasattr(target, 'lora_Cu') and adapter_name in target.lora_Cu:
                        target_lora_Cu = target.lora_Cu[adapter_name]
                        lora_Cus = []
                        for adapter, weight in zip(adapters, weights):
                            if adapter in target.lora_Cu:
                                lora_Cu_weight = target.lora_Cu[adapter].weight.data
                                lora_Cus.append(lora_Cu_weight * weight * target.scaling[adapter])
                        if len(lora_Cus) > 0:
                            merged_lora_Cu = torch.cat(lora_Cus, dim=0) # error from how to merge
                            target_lora_Cu.weight.data = merged_lora_Cu
                        #print(f'target_lora_Cu shape after merging is: {target_lora_Cu.weight.shape}')
                        #print('merged_lora_Cu is: ', merged_lora_Cu.shape)
                        #import time; time.sleep(10)
                    
                    # print('target_lora_A--',target_lora_A.shape)
                    # Merge DoRA magnitude vectors for "cat" combination
                    #print(f"target is {target}")
                    #print(target.keys())
                    #import time; time.sleep(10)
                    if hasattr(target, 'lora_magnitude_vector'):
                        #print("lora_magnitude_vector is: ", target.lora_magnitude_vector)
                        #print("adapter_name is: ", target.xy)
                        #import time; time.sleep(10000)
                        pass

                    if hasattr(target, 'lora_magnitude_vector') and adapter_name in target.lora_magnitude_vector and False:
                        print("INSIDE CAT COMBINATION TYPE OF ADD_WEIGHTED_ADAPTER")
                        target_dora = target.lora_magnitude_vector[adapter_name]
                        dora_magnitudes = []
                        dora_weights = []
                        
                        for adapter, weight in zip(adapters, weights):
                            if adapter in target.lora_magnitude_vector:
                                dora_layer = target.lora_magnitude_vector[adapter]
                                dora_mag = dora_layer.weight.data  # Shape: [out_features]
                                ...
                                # Weight the magnitude by adapter weight and scaling
                                # The magnitude vector represents the learned magnitude for each output feature
                                weighted_mag = dora_mag * weight * scaling
                                dora_magnitudes.append(weighted_mag)
                                dora_weights.append(weight)
                        
                        #print("dora_magnitudes----", dora_magnitudes)
                        #print(dora_magnitudes[0].shape)
                        #print("dora_weights----", dora_weights)
                        #import time; time.sleep(10000)
                        if len(dora_magnitudes) > 0:
                            # Since magnitude vectors have the same shape [out_features] for all adapters,
                            # we use weighted average (not concatenation) to merge them
                            # This preserves the per-output-feature magnitude information
                            total_weight = sum(dora_weights)
                            if total_weight > 0:
                                merged_dora_mag = sum(dora_magnitudes) / total_weight
                                #print("merged_dora_mag----", merged_dora_mag)
                                #import time; time.sleep(10000)
                            else:
                                merged_dora_mag = sum(dora_magnitudes) / len(dora_magnitudes)
                            
                            # Update the target DoRA layer's magnitude vector
                            #print("initial data of target_dora----", target_dora.weight.data)
                            #import time; time.sleep(10000)
                            target_dora.weight.data = merged_dora_mag
                        else:
                            import warnings
                            warnings.warn(
                                f"No DoRA magnitude vectors found in source adapters for merging. "
                                f"Keeping initialized magnitude vector for adapter {adapter_name}."
                            )
